/*
 * @Date: 2019-09-10 09:00:09
 * @LastEditors: yinht
 * @LastEditTime: 2020-12-12 15:58:31
 */
'use strict'
...
exports.cssLoaders = function (options) {
  ...
  function generateLoaders (loader, loaderOptions) {
    ...
    // load scss files for monitor center project
    if (loader === 'sass' && control.targetPages.includes('monitor')) {
      loaders.push({
        loader: 'sass-resources-loader',
        options: {
          sourceMap: options.sourceMap,
          resources: ['src/page/monitor/assets/style/*.scss']
        }
      })
    }
...
exports.getEntry = (globPath, targetPages) => {
  ...
  if (targetPages && targetPages instanceof Array) {
    ...
    if (
      process.env.NODE_ENV !== 'production' &&
      !targetPages.includes('index')
    ) {
      targetPages.push('index') // 不是生产环境则应保留目录页
    }
    // 判断字符串是否包含指定页面内容
    let judge = str => {
